...
class Day15 : DaySolver(15) {
    ...
    private val sensors = input.map { line ->
        ...
        Sensor(Vec2(sensorX.toInt(), sensorY.toInt()), Vec2(beaconX.toInt(), beaconY.toInt()))
    }.toMutableList()
    ...
    override fun calcPart2(): Long {
        val downLines = sensors.flatMap { s -> (s.pos.x - s.pos.y).let { listOf(it + s.dist + 1, it - s.dist - 1) } }
        val upLines = sensors.flatMap { s -> (s.pos.x + s.pos.y).let { listOf(it + s.dist + 1, it - s.dist - 1) } }

        for (downLine in downLines.sorted()) {
            for (upLine in upLines.sorted()) {
                if ((downLine + upLine) % 2 == 0) { // If we can't average them, there is no crossing
                    val x = (downLine + upLine) / 2 // The average of the 2 lines is where they cross
                    if (x < 0) continue
                    if (x > 4_000_000) break
                    val y = x - downLine
